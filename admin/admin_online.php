<table height="28" cellSpacing="0" cellPadding="0" width="100%" border="0">
      <tr align=center>
        ...
        Online : <b> <a href="./?act=online_m"><font color="#FFFFFF">Nh&#7853;p 
...
<?
	switch ($_GET['action'])
	{
		case 'del' :
			$id = $_GET['id'];
			$sql = "delete from online where providers_id='".$id."'";
...
<?
	if (isset($_POST['ButDel'])) {
		...
		{
			@$result = mysql_query("delete from online where providers_id='".$id."'",$con);
			if ($result) $cnt++;
		}
...
{	global $con;
	...
	$pageindex=taotrang("select count(*) from online","./?act=online"."&page=",$MAXPAGE,$page);
?>
...
<table border="1" cellpadding="2" style="border-collapse: collapse" bordercolor="#C9C9C9" width="100%" id="AutoNumber1">
  <tr>
    ...
    <td align="center" nowrap class="title"><b>Tên tr&#7921;c tuy&#7871;n</b></td>
    <td align="center" nowrap class="title"><b>Yahoo ID</b></td>
    <td align="center" nowrap class="title"><b>Không hi&#7875;n th&#7883;</b></td>
    <td align="center" nowrap class="title"><b>Ngày</b></td>
  </tr>
  
  <?
            	$sql="select *,DATE_FORMAT(providers_dateadded,'%d/%m/%Y %h:%m') as dateformat from online order by providers_id limit ".($p*$MAXPAGE).",".$MAXPAGE;
            	$result=mysql_query($sql,$con);
            	$i=0;
  ...
  <tr>
    ...
    <a href="./?act=online_m&page=<? echo $_REQUEST['page']; ?>&id=<? echo $row['providers_id']; ?>">S&#7917;a</a></td>
    <td width="20" bgcolor="<? echo $color; ?>" class="smallfont">
    <a onclick="return confirm('B&#7841;n có ch&#7855;c ch&#7855;n mu&#7889;n xoá ?');" href="./?act=online&action=del&page=<? echo $_REQUEST['page']; ?>&id=<? echo $row['providers_id']; ?>">Xoá</a></td>
    ...
    <td bgcolor="<? echo $color; ?>" class="smallfont"><? echo $row['providers_im']; ?>&nbsp;</td>
