...
if (isset($_POST['butSaveLoai'])) {
	...
	else
	{
		if (!empty($_POST['oldid'])) {
			$oldid = $_POST['oldid'];
			$sql = "update pro_banchay set products_id='".$products_id."', pro_sortorder='".$sort."', pro_status=".$status." where pro_id='".$oldid."'";
		}
		else 
			$sql = "insert into pro_banchay (products_id,pro_sortorder, pro_status) values ('".$products_id."','".$sort."',".$status.")";
		if (mysql_query($sql,$con)) {
			echo "<p align=center class='err'>&#272;ã c&#7853;p nh&#7853;t thành công</p><br>";
			echo "<script>window.location='index.php?act=pro_banchay&page=".$_REQUEST['page']."'</script>";
		}
...
<?
	if (isset($_GET['id'])) {
		$oldid=$_GET['id'];
		$sql = "select * from pro_banchay where pro_id='".$oldid."'";
...
<input type=hidden name="act" value="pro_banchay_m">
