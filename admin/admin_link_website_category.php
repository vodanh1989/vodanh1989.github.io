...
switch ($_GET['action']){
	case 'del' :
		$id = $_GET['id'];
		$result = mysql_query("select link_websites_categoriesid from link_websites where link_websites_categories_id>2 and link_websites_categoriesid='".$id."'",$con);
	...
	if (isset($_POST['ButDel'])) {
		...
		{
			@$result = mysql_query("delete from link_websites_categories where link_websites_categories_id>2 and link_websites_categories_id='".$id."'",$con);
			if ($result) $cnt++;
		}
...
<table border="1" cellpadding="2" style="border-collapse: collapse" bordercolor="#C9C9C9" width="100%" id="AutoNumber1">
  ...
  <?
            	$sql="select *,DATE_FORMAT(date_added,'%d/%m/%Y %h:%m') as dateformat from link_websites_categories where link_websites_categories_id>2 order by link_websites_categories_id limit ".($p*$MAXPAGE).",".$MAXPAGE;
            	$result=mysql_query($sql,$con);
            	$i=0;
